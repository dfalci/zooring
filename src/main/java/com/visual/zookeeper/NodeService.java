    ...
    class FingerTable{
        ...
        void fill(Map<String, byte[]> items){
            ...
            if (items != null || items.size() > 0)
                this.nodes.addAll(items.entrySet().stream().map(entry -> new NodeAddress(entry)).collect(Collectors.toList()));
            Collections.sort(this.nodes, (i1, i2) -> i1.id.compareTo(i2.id));
        }
